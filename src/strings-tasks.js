...
function isString(value) {
  if (typeof value === 'undefined' || value === null) {
    return false; // Если value undefined, то это не строка
  }
...
function removeFirstOccurrences(str, value) {
  ...
  if (index === -1) {
    return str; // Подстрока не найдена
  }
...
function invertCase(str) {
  ...
  arr.forEach((elem) => {
    newArr.push(
      elem.toUpperCase() === elem ? elem.toLowerCase() : elem.toUpperCase()
    );
  });
  return newArr.join('');
}
